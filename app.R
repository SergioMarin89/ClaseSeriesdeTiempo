...
ui <- fluidPage(theme = shinytheme("superhero"),
  ...
  navbarPage("Analisis serie de tiempo",
             tabPanel("Analisis descriptivo",
              ...
              numericInput('year','Ingrese ano de inicio',value =2017 ),
              tags$hr(),
              ...
              mainPanel(
                tabsetPanel(
                  ...
                  tabPanel("AnÃ¡lisis bÃ¡sico ST",
                          h4("GrÃ¡fico de la serie de tiempo"),
                          plotOutput("contents1"),

                          #Horizontal line ----
                          tags$hr(),

                          h4("ResÃºmen de los datos"),
...
server <- function(input, output) {
  ...
  data_l<-reactive({
    ...
    output$tendencia_desc_data <- renderPlot({
      ...
      plot(Tt,ylim=c(min(data_st),max(data_st)),main="Componente tendencia estimada por descomposicion
    ...
    output$tendencia_desc_logdata <- renderPlot({
      ...
      plot(Tt_log,ylim=c(min(data_st),max(data_st)),main="Componente tendencia estimada por descomposicion
      ...
      output$error_datos_logdatos <- renderPlot({
        ...
        plot(Tt_error_datos,main="Componente error estimada por descomposicion clasica aditiva")
      ...
      output$error_datos_logdatos_multi <- renderPlot({
        ...
        plot(error_datos_logdatos_multi,main="Componente error estimada por descomposicion clasica multiplicativa")
